...
async function fillEventInformation(bookings) {
  await Promise.all(
    await bookings.map(async booking => {
      ...
      bookingCopy.event.thumbnail = null;
      return bookingCopy;
    })
  );
...
exports.updateBooking = async (
  ...
  if (
    oldBooking.seats !== booking.seats &&
    Number(booking.seats) >
      Number(tempEvent.freeSeats) + Number(oldBooking.seats)
  ) {
    return new Error("There are no free seats :(");
  }

  if (
    (oldBooking.status === "ATTENDED" && booking.status === "ATTENDED") ||
...
exports.getByEventId = async eventId =>
  mongo.find(col, { eventId: String(eventId), status: { $ne: "CANCELED" } });

exports.getByUserId = async userId =>
  mongo.find(col, { ownerId: String(userId) });
